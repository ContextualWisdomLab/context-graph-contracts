...
3. Require the exact-head `reproducibility` workflow to build wheel and source distribution twice from two independent clean checkouts of the same commit with the same commit-derived `SOURCE_DATE_EPOCH`, then fail closed unless artifact filenames and SHA-256 byte identities are identical. Retain the exact-SHA reproducibility evidence; one successful package build is not reproducibility proof.
4. Install the built package into an isolated environment, run `cwl-context-conformance`, and require a passing report from the installed package rather than a source-tree import.
5. Run `cwl-context-conformance-manifest` from that same isolated installation and retain the exact distribution version plus every packaged semantic-profile SHA-256 digest with the release evidence. Then run `cwl-context-conformance-admit` against the independently approved manifest expected for the release and require exit `0`, `admitted=true`, a passing semantic report, and an exact manifest-verification result. This composite comparison proves installed semantic behavior plus package/version/profile byte identity only; it does not replace review, signature, provenance, SBOM, or authorization gates.
6. Require the protected-main supply-chain run to build the wheel and source distribution, generate the canonical SPDX 3.0.1 SBOM, generate `SHA256SUMS`, create SLSA provenance and explicit `https://spdx.dev/Document/v3` SBOM attestations for the exact package artifacts, and immediately verify both predicate classes for each wheel/source distribution against the exact repository, `refs/heads/main`, source SHA, signer workflow/digest, GitHub Actions OIDC issuer, and hosted-runner policy. For the custom SPDX assertion, also require at least one verified statement whose parsed signed predicate exactly equals the downloaded canonical SPDX document; predicate-type equality alone is not SBOM identity. Retain the machine-readable verification results under the exact source SHA.
7. Download that exact package-evidence bundle and run `cwl-context-release-evidence-admit <evidence-directory> <approved-conformance-manifest> <approved-contract-bundle-manifest>`. Require exit `0`, `admitted=true`, an exact installed semantic/complete-bundle admission, a passing wheel/source/SPDX/checksum verification, and equality between the verified package distribution version and the installed approved distribution version. This rejects evidence splicing across otherwise coherent release versions; it still does not authenticate the bundle or replace artifact attestation, protected-main source identity, independent review, or release authorization.
8. Independently verify the built wheel and source distribution attestations before publication even when the repository's protected-main verifier already passed. Require the intended repository/ref/source digest/signer identity for SLSA provenance and the canonical SPDX 3 predicate, and compare the parsed signed SPDX predicate with the retained canonical SPDX document rather than accepting a matching predicate type or an unrelated valid signature.
9. Update `CHANGELOG.md` for the exact version and verify package metadata/version agree with the intended tag.
10. Publish/tag only after all preceding evidence still refers to the unchanged integrated commit. Record source and artifact SHA-256 digests, exact-SHA reproducibility evidence, the exact conformance manifest, installed release-admission result, complete release-evidence admission result, retained attestation-verification JSON, and independent provenance/authorization evidence with the release.
...
3. Re-verify its `SHA256SUMS`, SLSA provenance attestation, and SPDX 3 attestation against the exact original protected-main source/signer identity, then require the verified signed SPDX predicate to match the retained canonical SPDX document for that rollback version.
