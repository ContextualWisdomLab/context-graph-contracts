...
| `main` integration/default governance | External governance / open | Program architecture requires `main` as intended integration/default. Live repository metadata still reports `default_branch=develop`; both `develop` and `main` are now protected. Central owner path `ContextualWisdomLab/.github#1137` owns the remaining default-branch transition and effective `~DEFAULT_BRANCH` ruleset reread. | Change repository default to the already-protected `main`, immediately re-read effective rulesets and required workflows, preserve temporary `develop` protection for the transition, then rebuild the PR stack from fresh protected `main` without transferring predecessor evidence. |
