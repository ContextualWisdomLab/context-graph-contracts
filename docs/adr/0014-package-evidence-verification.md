...
2. requires `SHA256SUMS` to name exactly one wheel, one `.tar.gz` source distribution, and `cwl-context-contracts.spdx.json`;
...
6. checks that the SBOM retains the repository workflow's SPDX 3.0.1 context, `CreationInfo` specification version, and software-package evidence shape; and
...
Consumers can repeat the package bundle's local integrity checks with the same installed toolchain used for semantic and complete-contract verification. Path traversal, symlink substitution, checksum omission, checksum drift, and malformed SPDX evidence fail closed.
...
- RED acceptance: `tests/test_package_evidence_verifier.py` was committed before the production verifier existed.
