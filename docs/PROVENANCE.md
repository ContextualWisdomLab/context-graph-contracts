...
- require at least one verified SPDX attestation for each package artifact whose parsed signed predicate exactly equals the downloaded canonical SPDX document, rejecting missing/non-object evidence, duplicate JSON members, non-standard numeric constants, malformed UTF-8/JSON, or predicate drift instead of treating predicate type alone as SBOM identity; and
...
4. Compare the parsed signed SPDX predicate from a verified package attestation with the exact downloaded canonical SPDX 3.0.1 document retained in package evidence. Predicate-type equality alone does not prove that the signed SBOM is the SBOM whose bytes were checksummed and retained.
...
- A protected-main `gh attestation verify` identity/predicate-type pass is insufficient by itself for the custom SPDX assertion: the verifier also requires the signed SPDX predicate to match the downloaded canonical SBOM. This still does not supply the human review or release authorization that made the source eligible for `main` in the first place.
