...
- before querying GitHub, snapshot each package artifact through a stable regular-file descriptor and compute its SHA-256 identity, then require the verified in-toto `statement.subject` for both provenance and SPDX checks to include that exact digest, preventing provenance/SPDX evidence from being spliced across different package bytes;
...
- require at least one **subject-matched** verified SPDX statement for each package artifact whose parsed signed predicate exactly equals the downloaded canonical SPDX document, rejecting missing/non-object evidence, duplicate JSON members, non-standard numeric constants, malformed UTF-8/JSON, artifact-subject drift, or predicate drift instead of treating predicate type alone as SBOM identity; and
...
3. Confirm the attested source repository, `refs/heads/main`, source digest, signer workflow/digest, GitHub Actions OIDC issuer, and hosted-runner provenance match the intended release evidence. For every accepted statement, also confirm its in-toto `subject` carries the SHA-256 digest of the exact local wheel or source distribution being admitted. The repository's protected-main workflow performs the same fail-closed checks, but a consumer with its own trust policy should still verify independently.
4. Compare the parsed signed SPDX predicate from a **subject-matched** verified package attestation with the exact downloaded canonical SPDX 3.0.1 document retained in package evidence. Predicate-type equality alone does not prove that the signed SBOM is the SBOM whose bytes were checksummed and retained.
...
- A protected-main `gh attestation verify` identity/predicate-type pass is insufficient by itself: the verifier also requires the verified statement subject to match the snapshotted package bytes, and for the custom SPDX assertion requires the **same subject-matched statement's** signed predicate to match the downloaded canonical SBOM. These checks still do not supply the human review or release authorization that made the source eligible for `main` in the first place.
