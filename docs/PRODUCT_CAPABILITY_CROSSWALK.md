...
| External data-management framework mapping and result exchange | Framework metadata, CWL-authored capability/evidence/profile identifiers, exact basis-point assessment results, readiness/missing-evidence state, canonical tenant/subject/truth/provenance/time references, and explicit license classification | `data-management-framework.schema.json`, `data-management-assessment.schema.json`, public positive fixtures, schema regressions, complete-bundle byte evidence | Licensed framework content, scoring algorithms/thresholds, certification, and authoritative assessment evidence/initiative state belong to the publisher rights model and owning CWL products |
...
- **Semantic Data Portal** owns catalog assets, glossary, data products, contracts, trust/certification, graph/search/MCP projections and authoritative Data/AI context including assessment evidence.
