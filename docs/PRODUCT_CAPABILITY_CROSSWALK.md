...
| External data-management framework mapping | Framework metadata, CWL-authored capability/evidence/profile identifiers, canonical truth/authority/provenance references, and explicit license classification | `data-management-framework.schema.json`, schema regressions, complete-bundle byte evidence | Licensed framework content, maturity scoring, certification, and authoritative evidence/initiative state belong to the publisher rights model and owning CWL products |
